        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12'>
            {
                exclusiveOffers.map((item)=>(
                    <div key={item._id} className='group relative flex flex-col items-start justify-center gap-1 pt-12 md:pt-18 px-4 rounded-xl text-white bg-no-repeat bg-coverb bg-center'
                    style={{backgroundImage: `url(${item.image})`}}>
                        ...
                        <button className=' group flex  items-center gap-2 font-medium cursor-pointer mt-4 mb-5'>View All Offers
                            <img src={assets.arrowIcon} className='invert group-hover:translate-x-1 transition-all'></img>
                         </button>
                    </div>
                )) }
        </div>
